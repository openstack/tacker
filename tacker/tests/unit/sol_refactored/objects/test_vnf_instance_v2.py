...
class TestVnfInstanceObject(tests_base.BaseTestCase):
    ...
    def test_from_db_obj(self, mock_decrypt):
        ...
        dict_inst = {
            "id": "test_id",
            "vnfInstanceName": None,
            "vnfInstanceDescription": None,
            "vnfdId": "test_vnfd_id",
            "vnfProvider": "test_provider",
            "vnfProductName": "test_product",
            "vnfSoftwareVersion": "1.0.0",
            "vnfdVersion": "1.0.0",
            "vnfConfigurableProperties": None,
            "vimConnectionInfo": vim_infos,
            "instantiationState": "INSTANTIATED",
            "instantiatedVnfInfo": None,
            "metadata": None,
            "extensions": None,
            "_links": _link
        }
        obj = objects.VnfInstanceV2.from_db_obj(dict_inst)
        dict_of_objects = {}
    ...
    def test_to_db_obj(self, mock_encrypt):
        ...
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["password"], _encrypted_data)
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["bearer_token"], _encrypted_data)
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["client_secret"], _encrypted_data)
    ...
    def test_to_db_obj_no_encrypt(self):
        ...
        o_dict = jsonutils.loads(obj['vimConnectionInfo'])
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["password"], _string_data)
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["bearer_token"], _string_data)
        self.assertEqual(
            o_dict["vim1"]["accessInfo"]["client_secret"], _string_data)
